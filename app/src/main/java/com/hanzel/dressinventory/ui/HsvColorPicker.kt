...
/** Nearest name from the 1,566-entry NTC dataset (RGB + HSL blended distance). */
fun colorDisplayName(hex: Long): String = ntcName(hex)

/** Nearest entry from the curated chart — used for outfit-matching logic. */
...
/**
 * HSV colour picker:
 *  - Outer rainbow ring  → selects Hue (0–360°)
 *  - Inner square        → selects Saturation (x) and Value/brightness (y)
 *
 * [onColorChanged] is invoked with (hexLong, colourName) on every change.
 * Colour names come from the 1,566-entry NTC dataset.
 */
@Composable
fun HsvColorPicker(
    initialHex: Long,
    ...
    BoxWithConstraints(
        ...
        val innerR = outerR * (1f - ringFrac * 2)
        val squareHalf = innerR / sqrt(2f)
